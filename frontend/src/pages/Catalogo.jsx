        ...
        <div className="mb-10 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="relative w-full md:max-w-sm">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-700 bg-slate-900 py-3 pl-11 pr-4 text-white placeholder-slate-500 outline-none transition focus:border-cyan-400"
            />
          </div>
        ...
        {loading && (
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="h-96 animate-pulse rounded-3xl border border-slate-800 bg-slate-900"
              />
            ))}
          </div>
        )}
          ...
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {filteredProducts.map((product) => {
              ...
              return (
                <div
                  key={product.id}
                  className="group overflow-hidden rounded-3xl border border-slate-800 bg-slate-900 transition-all duration-500 hover:-translate-y-2 hover:border-cyan-500 hover:shadow-[0_0_35px_rgba(34,211,238,0.15)]"
                >
                  <div
                  ...
                  <div className="p-7">
                    ...
                    <h3 className="mt-5 text-2xl font-bold">{product.nombre}</h3>
                    ...
                    <div className="mt-6 flex items-center justify-between">
                      ...
                      {product.stock === 0 ? (
                        <span className="rounded-xl border border-slate-700 px-5 py-3 text-sm text-slate-500">
                          Agotado
                        </span>
                      ) : (
                        <button
                          ...
                          className="rounded-xl bg-gradient-to-r from-cyan-500 to-violet-600 px-5 py-3 transition duration-300 hover:scale-105"
                        >
                          Personalizar
                        </button>
                      )}
